...
from espnet.nets.pytorch_backend.e2e_asr_mix import encoder_for, PIT
...
class Reporter(chainer.Chain):
    ...
    def report(self, loss_ctc, loss_tr, acc, cer, wer, mtl_loss):
        """Define reporter."""
        reporter.report({'loss_ctc': loss_ctc}, self)
        reporter.report({'loss_tr': loss_tr}, self)
        reporter.report({'acc': acc}, self)
        reporter.report({'cer': cer}, self)
        reporter.report({'wer': wer}, self)
        logging.info('loss:' + str(loss_tr))
...
class E2E(E2E_ASR, ASRInterface, torch.nn.Module):
    ...
    def add_arguments(parser):
        ...
        group.add_argument('--dropout-rate-embed-decoder', default=0.0, type=float,
                           help='Dropout rate for the decoder embedding')
    ...
    def forward(self, xs_pad, ilens, ys_pad):
        ...
        for i in range(ys_pad.size(1)):  # B
            ys_pad[:, i] = ys_pad[min_perm[i], i]
        ret = [self.dec(hs_pad[i], hlens[i], ys_pad[i]) for i in range(self.num_spkrs)]
        loss_rnnt = torch.mean(torch.stack([r[0] for r in ret], dim=0).to(ret[0].device))  # (B)
        ...
        if self.training or not (self.report_cer or self.report_wer) or not isinstance(hs_pad, list):
            ...
            if self.recog_args.ctc_weight > 0.0:
                lpz = [self.dec.log_softmax(hs_pad[i]).data for i in range(self.num_spkrs)]
            else:
                lpz = None
            ...
            y_hats = [[nbest_hyp[0]['yseq'][1:] for nbest_hyp in nbest_hyps[i]] for i in range(self.num_spkrs)]
        ...
        if loss_data < CTC_LOSS_THRESHOLD and not math.isnan(loss_data):
            self.reporter.report(loss_rnnt_data, loss_att_data, None, cer, wer, loss_data)
        else:
    ...
    def recognize(self, x, recog_args, char_list, rnnlm=None):
        ...
        #     lpz = [self.dec.log_softmax(i)[0] for i in hs]
        # else:
        lpz = None

        # 2. decoder
        # decode the first utterance
        y = [self.dec.recognize_beam(hs[i][0], recog_args, rnnlm) for i in range(self.num_spkrs)]
    ...
    def recognize_batch(self, xs, recog_args, char_list, rnnlm=None):
        ...
        # calculate log P(z_t|X) for CTC scores
        if recog_args.ctc_weight > 0.0:
            lpz = [self.dec.log_softmax(hs_pad[i]) for i in range(self.num_spkrs)]
